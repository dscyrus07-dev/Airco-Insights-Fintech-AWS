...
# Initialize the unified IDFC classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> IDFCClassifier:
    """Get or create the unified IDFC classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = IDFCClassifier()
        logger.info("IDFC classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    try:
        ...
        logger.debug("IDFC learning-store update skipped", exc_info=True)
    return result
