...
class CanaraReconciliation:
    """Balance reconciliation engine for Canara Bank transactions."""
    ...
    def reconcile(
        ...
        result = CanaraReconciliationResult(
        ...
        if not is_reconciled and self.strict_mode:
            if mismatches:
                ...
                raise CanaraReconciliationError(
                    ...
                    details={"transaction_index": m.transaction_index,
                             "expected": m.expected_balance, "actual": m.actual_balance},
                )
            raise CanaraReconciliationError(
                f"Final balance mismatch: calc={calculated_closing:.2f} actual={closing_balance:.2f}",
                error_code="FINAL_BALANCE_MISMATCH",
                details={"calculated": calculated_closing, "actual": closing_balance},
            )
        ...
        for i, txn in enumerate(transactions):
            ...
            if abs(expected - curr_bal) > self.TOLERANCE:
                ...
                if abs(alt - curr_bal) < abs(expected - curr_bal):
                    ...
                    corrections += 1
                    self.logger.debug("Corrected txn %d: swapped debit/credit", i)
            corrected.append(txn_copy)
        return corrected, corrections


__all__ = [
    "CanaraReconciliationError", "ReconciliationMismatch",
    "CanaraReconciliationResult", "CanaraReconciliation",
]
