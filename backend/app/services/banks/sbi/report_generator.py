...
Uses SBIClassifier for transaction categorization.
"""
...
def get_classifier() -> SBIClassifier:
    """Get SBI classifier instance."""
    return SBIClassifier()


def classify(row) -> Tuple[str, int]:
    """Classify a transaction row using the shared base classifier path."""
    return _base_classify(row, classifier_factory=get_classifier, bank_name='SBI')
...
def generate_report(
    ...
    """Generate the shared HDFC-style report for SBI statements."""
