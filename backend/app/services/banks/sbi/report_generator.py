...
def _build_source_analysis_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    recurring_keys = set()
    source_groups = frame.groupby(frame["Source"].fillna("").astype(str).str.lower().fillna("unknown"))
    ...
    frame["IsRecurring"] = frame["Source"].fillna("").astype(str).str.lower().fillna("unknown").isin(recurring_keys)
...
# ============================================================================
# HELPER FUNCTIONS FOR FORMULA EXCEL ENGINE
# ============================================================================
...
def _build_category_outcome_tables(source_frame: pd.DataFrame) -> Dict[str, Any]:
    ...
    for period in month_dt.dropna().dt.to_period("M").tolist():
        if period not in month_periods:
            month_periods.append(period)
    month_periods = sorted(month_periods, key=lambda p: p.ordinal)

    monthly_tables = {}
    for period in month_periods:
        period_str = period.strftime("%b-%y")
        month_df = frame[frame["MonthKey"] == period_str].copy()
        if len(month_df) == 0:
            continue

        category_agg = (
            month_df.groupby("Category")
            .agg(
                CreditCount=("Credit", lambda s: int((s > 0).sum())),
                DebitCount=("Debit", lambda s: int((s > 0).sum())),
                TotalCredit=("Credit", "sum"),
                TotalDebit=("Debit", "sum"),
            )
            .reset_index()
        )
        category_agg = category_agg[category_agg["Category"] != "Others"].copy()

        source_agg = (
            month_df.groupby("Source")
            .agg(
                CreditCount=("Credit", lambda s: int((s > 0).sum())),
                DebitCount=("Debit", lambda s: int((s > 0).sum())),
                TotalCredit=("Credit", "sum"),
        ...
        source_agg = source_agg[source_agg["Source"] != "Unknown"].copy()

        monthly_tables[period_str] = {
            "category": category_agg.to_dict("records"),
            "source": source_agg.to_dict("records"),
        }
