...
Uses HDFCClassifier for transaction categorization.
"""
...
def get_classifier() -> HDFCClassifier:
    """Get HDFC classifier instance."""
    return HDFCClassifier()


def classify(row) -> Tuple[str, int]:
    """Classify a transaction row using the shared base classifier path."""
    return _base_classify(row, classifier_factory=get_classifier, bank_name='HDFC')
...
def generate_report(
    ...
    """Generate the shared HDFC-style report for HDFC statements."""
