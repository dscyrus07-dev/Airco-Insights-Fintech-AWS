...
===============================================
Full-grade analytics: category breakdown, monthly + weekly summaries,
...
class AggregationResult:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "debit_categories": [{"category": c.category, "total": c.total_amount,
                                   "count": c.transaction_count, "avg": c.avg_amount,
                                   "percentage": c.percentage} for c in self.debit_categories],
            "credit_categories": [{"category": c.category, "total": c.total_amount,
                                    "count": c.transaction_count, "avg": c.avg_amount,
                                    "percentage": c.percentage} for c in self.credit_categories],
...
class IDFCAggregationEngine:
    ...
    def aggregate(self, transactions: List[Dict[str, Any]],
        ...
        if not transactions:
            return AggregationResult(opening_balance=opening or 0, closing_balance=closing or 0,
                                     ...
                                     monthly_summary={})
        self.logger.info("Aggregating %d IDFC transactions", len(transactions))
        ...
        debit_cats    = self._aggregate_categories(transactions, True)
        credit_cats   = self._aggregate_categories(transactions, False)
        monthly_sums  = self._aggregate_monthly(transactions)
        ...
        wc, wd = self._aggregate_weekly(transactions)
        rs     = self._aggregate_recurring(transactions)
    ...
    def _aggregate_categories(self, transactions: List[Dict], is_debit: bool) -> List[CategorySummary]:
        ...
        for t in transactions:
            if is_debit and t.get("debit"):
                data[t.get("category", "Others Debit")]["total"] += t["debit"]
                data[t.get("category", "Others Debit")]["count"] += 1
            elif not is_debit and t.get("credit"):
                data[t.get("category", "Others Credit")]["total"] += t["credit"]
                data[t.get("category", "Others Credit")]["count"] += 1
        ...
        sums = [CategorySummary(cat, round(d["total"],2), d["count"],
                                ...
                                round(d["total"]/grand*100,1) if grand else 0)
    ...
    def _aggregate_monthly(self, transactions: List[Dict]) -> List[MonthlySummary]:
        ...
        for t in transactions:
            ...
            if not ds: continue
            try:
                mk = str(ds)[:7] if "-" in str(ds) else f"20{str(ds).split('/')[2]}-{str(ds).split('/')[1]}"
    ...
    def _aggregate_weekly(self, transactions: List[Dict]) -> tuple:
        ...
        for t in transactions:
            ...
            try:
                ...
                y,w,_ = dt.isocalendar(); wk = f"{y}-W{w:02d}"
