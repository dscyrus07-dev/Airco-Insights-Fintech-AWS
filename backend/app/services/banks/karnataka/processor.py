...
@dataclass
class KarnatakaProcessingMetrics(GenericProcessingMetrics):
    ai_classified_count: int = 0
    integrity_passed: bool = False


@dataclass
class KarnatakaProcessingResult(GenericProcessingResult):
...
class KarnatakaProcessor:
    def __init__(self, strict_mode: bool = True, enable_ai: bool = False, api_key: Optional[str] = None, audit_service=None, job_id: Optional[str] = None):
        ...
        self.excel_generator = KarnatakaExcelGenerator()
        self.formula_excel_engine = KarnatakaFormulaExcelEngine()
        self.integrity_guard = DataIntegrityGuard(strict_mode=strict_mode)
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")

    def process(self, file_path: str, user_info: Dict[str, Any], output_dir: Optional[str] = None) -> KarnatakaProcessingResult:
        pipeline_start = time.monotonic()
        metrics = KarnatakaProcessingMetrics()
        try:
            ...
            if parse_result.total_count <= 0:
                raise KarnatakaParseError("Could not extract transactions from this Karnataka Bank PDF.", error_code="NO_TRANSACTIONS")
            ...
            excel_path = os.path.join(output_dir, f"karnataka_{uuid.uuid4().hex[:12]}.xlsx")
            ...
            metadata = {
                ...
                "opening_balance": structure_result.metadata.opening_balance or parse_result.opening_balance or 0,
            ...
            try:
                ...
                self.logger.warning("Formula Excel generation failed for Karnataka, falling back to legacy generator", exc_info=True)
                ...
                try:
                    self.audit_service.finalize_job_audit(
                        ...
                        statement_header={
                            ...
                            "bankname": getattr(self.parser, "BANK_NAME", "Karnataka"),
            ...
            return KarnatakaProcessingResult(
                status="success",
                excel_path=excel_path,
            ...
            self.logger.error("Karnataka Bank processing failed", exc_info=True)
            if isinstance(exc, PDFIntegrityError):
                error_code = exc.error_code
            elif isinstance(exc, (KarnatakaStructureError, KarnatakaParseError, KarnatakaValidationError, KarnatakaReconciliationError)):
    ...
    def _time_step(self, step_name: str, func, metrics: KarnatakaProcessingMetrics):
        started = time.time()
        result = func()
...
def generate_report(transactions, output_path, user_info, metadata=None):
    from .report_generator import generate_report as generate_karnataka_report

    return generate_karnataka_report(transactions, output_path, user_info, metadata)
