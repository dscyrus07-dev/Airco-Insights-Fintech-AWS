"""
Airco Insights — IDFC Bank Recurring Engine
=============================================
Full-grade recurring detection: merchant grouping, weekly/monthly/quarterly
frequency detection, amount-consistency tolerance, known-pattern matching.
"""
...
class IDFCRecurringEngine:
    ...
    WEEKLY_RANGE = (5, 9); MONTHLY_RANGE = (25, 35); QUARTERLY_RANGE = (85, 95)
    ...
    def detect(self, transactions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for txn in transactions:
            t   = dict(txn)
            mk  = self._merchant_key(txn)
            known = self._known_pattern(txn)
            if known:
                t["is_recurring"] = True; t["recurring_type"] = known[0]; t["recurring_frequency"] = known[1]
            elif mk in patterns:
                p = patterns[mk]
                t["is_recurring"] = True; t["recurring_type"] = p.recurring_type; t["recurring_frequency"] = p.frequency
            else:
                t["is_recurring"] = False; t["recurring_type"] = None; t["recurring_frequency"] = None
            t["recurring"] = "Yes" if t["is_recurring"] else "No"
            result.append(t)
        self.logger.info("IDFC recurring detected: %d", sum(1 for t in result if t.get("is_recurring")))
        return result

    def _group_by_merchant(self, txns: List[Dict]) -> Dict[str, List[Dict]]:
    ...
    def _merchant_key(self, txn: Dict) -> str:
    ...
    def _detect_patterns(self, groups: Dict[str, List[Dict]]) -> Dict[str, RecurringPattern]:
        ...
        for key, txns in groups.items():
            ...
            for i in range(1, len(sorted_t)):
                try:
                    d0 = datetime.strptime(sorted_t[i-1]["date"], "%Y-%m-%d")
                    d1 = datetime.strptime(sorted_t[i]["date"],   "%Y-%m-%d")
                    delta = (d1 - d0).days
    ...
    def _type(self, key: str, txns: List[Dict]) -> str:
    ...
    def _known_pattern(self, txn: Dict) -> Optional[Tuple[str, str]]:
        desc = (txn.get("description") or "").upper()
        for p in self._emi:
