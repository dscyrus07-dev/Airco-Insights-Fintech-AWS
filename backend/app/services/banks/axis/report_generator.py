...
Uses AxisClassifier for transaction categorization.
"""
...
def get_classifier() -> AxisClassifier:
    """Get Axis classifier instance."""
    return AxisClassifier()


def classify(row) -> Tuple[str, int]:
    """Classify a transaction row using the shared base classifier path."""
    return _base_classify(row, classifier_factory=get_classifier, bank_name='Axis')
...
def generate_report(
    ...
    """Generate the shared HDFC-style report for Axis statements."""
