...
# Initialize the unified Canara classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> CanaraClassifier:
    """Get or create the unified Canara classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = CanaraClassifier()
        logger.info("Canara classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    try:
        ...
        logger.debug("Canara learning-store update skipped", exc_info=True)
    return result
