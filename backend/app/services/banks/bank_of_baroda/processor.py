...
from .parser import BankOfBarodaParser, BankOfBarodaParseResult, BankOfBarodaTransaction
...
@dataclass
class BankOfBarodaProcessingMetrics(GenericProcessingMetrics):
    ai_classified_count: int = 0
    integrity_passed: bool = False


@dataclass
class BankOfBarodaProcessingResult(GenericProcessingResult):
...
class BankOfBarodaProcessor:
    def __init__(self, strict_mode: bool = True, enable_ai: bool = False, api_key: Optional[str] = None, audit_service=None, job_id: Optional[str] = None):
        ...
        self.excel_generator = BankOfBarodaExcelGenerator()
        self.formula_excel_engine = BankOfBarodaFormulaExcelEngine()
        self.integrity_guard = DataIntegrityGuard(strict_mode=strict_mode)
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")

    def process(self, file_path: str, user_info: Dict[str, Any], output_dir: Optional[str] = None) -> BankOfBarodaProcessingResult:
        pipeline_start = time.monotonic()
        metrics = BankOfBarodaProcessingMetrics()
        try:
            ...
            if parse_result.total_count <= 0:
                raise GenericParseError("Could not extract transactions from this Bank of Baroda PDF.", error_code="NO_TRANSACTIONS")

            validated, validation_stats = self._time_step(
            ...
            if self.enable_ai and self.api_key and self.ai_fallback and unclassified:
                ...
                for txn in processed_transactions:
                    if str(txn.get("category", "")).startswith("Others"):
                        merged_transactions.append(next(ai_iter, txn))
                    else:
            ...
            excel_path = os.path.join(output_dir, f"bank_of_baroda_{uuid.uuid4().hex[:12]}.xlsx")
            ...
            self.logger.error("Bank of Baroda processing failed", exc_info=True)
    ...
    def _time_step(self, step_name: str, func, metrics: BankOfBarodaProcessingMetrics):
        started = time.time()
        result = func()
