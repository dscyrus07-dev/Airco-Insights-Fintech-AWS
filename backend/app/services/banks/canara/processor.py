...
from .parser import CanaraParser, CanaraParseResult, CanaraTransaction
...
@dataclass
class CanaraProcessingMetrics(GenericProcessingMetrics):
    ai_classified_count: int = 0
    integrity_passed: bool = False


@dataclass
class CanaraProcessingResult(GenericProcessingResult):
...
class CanaraProcessor:
    def __init__(self, strict_mode: bool = True, enable_ai: bool = False, api_key: Optional[str] = None, audit_service=None, job_id: Optional[str] = None):
        ...
        self.excel_generator = CanaraExcelGenerator()
        self.formula_excel_engine = CanaraFormulaExcelEngine()
        self.integrity_guard = DataIntegrityGuard(strict_mode=strict_mode)
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")

    def process(self, file_path: str, user_info: Dict[str, Any], output_dir: Optional[str] = None) -> CanaraProcessingResult:
        pipeline_start = time.monotonic()
        metrics = CanaraProcessingMetrics()
        try:
            ...
            if parse_result.total_count <= 0:
                raise GenericParseError("Could not extract transactions from this Canara PDF.", error_code="NO_TRANSACTIONS")
            ...
            excel_path = os.path.join(output_dir, f"canara_{uuid.uuid4().hex[:12]}.xlsx")
            ...
            try:
                ...
                self.logger.warning("Formula Excel generation failed for Canara, falling back to legacy generator", exc_info=True)
                ...
                try:
                    self.audit_service.finalize_job_audit(
                        ...
                        statement_header={
                            ...
                            "bankname": getattr(self.parser, "BANK_NAME", "Canara"),
            ...
            self.logger.error("Canara processing failed", exc_info=True)
    ...
    def _time_step(self, step_name: str, func, metrics: CanaraProcessingMetrics):
        started = time.time()
        result = func()
