...
from .parser import PaytmParser, PaytmParseResult, PaytmTransaction
...
@dataclass
class PaytmProcessingMetrics(GenericProcessingMetrics):
    ai_classified_count: int = 0
    integrity_passed: bool = False


@dataclass
class PaytmProcessingResult(GenericProcessingResult):
...
class PaytmProcessor:
    def __init__(self, strict_mode: bool = True, enable_ai: bool = False, api_key: Optional[str] = None, audit_service=None, job_id: Optional[str] = None):
        ...
        self.excel_generator = PaytmExcelGenerator()
        self.formula_excel_engine = PaytmFormulaExcelEngine()
        self.integrity_guard = DataIntegrityGuard(strict_mode=strict_mode)
        self.logger = logging.getLogger(f"{__name__}.{self.__class__.__name__}")

    def process(self, file_path: str, user_info: Dict[str, Any], output_dir: Optional[str] = None) -> PaytmProcessingResult:
        pipeline_start = time.monotonic()
        metrics = PaytmProcessingMetrics()
        try:
            ...
            if parse_result.total_count <= 0:
                raise GenericParseError("Could not extract transactions from this Paytm PDF.", error_code="NO_TRANSACTIONS")
            ...
            excel_path = os.path.join(output_dir, f"paytm_{uuid.uuid4().hex[:12]}.xlsx")
            ...
            try:
                ...
                self.logger.warning("Formula Excel generation failed for Paytm, falling back to legacy generator", exc_info=True)
                ...
                try:
                    self.audit_service.finalize_job_audit(
                        ...
                        statement_header={
                            ...
                            "bankname": getattr(self.parser, "BANK_NAME", "Paytm"),
            ...
            self.logger.error("Paytm processing failed", exc_info=True)
    ...
    def _time_step(self, step_name: str, func, metrics: PaytmProcessingMetrics):
        started = time.time()
        result = func()
