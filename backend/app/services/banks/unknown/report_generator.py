...
# Initialize the unified Unknown classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> UnknownClassifier:
    """Get or create the unified Unknown classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = UnknownClassifier()
        logger.info("Unknown classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    try:
        ...
        logger.debug("Unknown learning-store update skipped", exc_info=True)
    return result
