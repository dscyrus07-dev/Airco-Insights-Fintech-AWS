...
# Initialize the unified Paytm classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> PaytmClassifier:
    """Get or create the unified Paytm classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = PaytmClassifier()
        logger.info("Paytm classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    try:
        ...
        logger.debug("Paytm learning-store update skipped", exc_info=True)
    return result
