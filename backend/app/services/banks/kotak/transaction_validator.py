"""
Airco Insights — Kotak Bank Transaction Validator
==================================================
Validates and normalizes Kotak Bank transactions.
...
class KotakValidationError(Exception):
...
class KotakTransactionValidator:
    """Validates and normalizes Kotak Bank transactions."""

    # Kotak date formats (matches HDFC robustness)
    DATE_FORMATS = [
        "%d/%m/%y",      # DD/MM/YY
        "%d/%m/%Y",      # DD/MM/YYYY
        "%d-%m-%y",      # DD-MM-YY
    ...
    def validate(self, transactions) -> KotakValidationResult:
        if not transactions:
            raise KotakValidationError("No transactions to validate", error_code="NO_TRANSACTIONS")
        ...
        self.logger.info("Validated %d/%d Kotak transactions", len(validated), len(transactions))

        return KotakValidationResult(
