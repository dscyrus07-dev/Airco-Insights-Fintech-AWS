...
# Initialize the unified Karnataka classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> KarnatakaClassifier:
    """Get or create the unified Karnataka classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = KarnatakaClassifier()
        logger.info("Karnataka classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    try:
        ...
        logger.debug("Karnataka learning-store update skipped", exc_info=True)
    return result
