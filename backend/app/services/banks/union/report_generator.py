...
# Initialize the unified Union classifier (singleton)
_classifier = None
_learning_store = None

def get_classifier() -> UnionClassifier:
    """Get or create the unified Union classifier instance."""
    global _classifier
    if _classifier is None:
        _classifier = UnionClassifier()
        logger.info("Union classifier initialized: %s", _classifier.get_category_stats())
    return _classifier
...
def classify(row) -> Tuple[str, int]:
    ...
    try:
        ...
        logger.debug("Union learning-store update skipped", exc_info=True)
    return result
