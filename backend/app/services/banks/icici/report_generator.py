...
Uses ICICIClassifier for transaction categorization.
"""
...
def get_classifier() -> ICICIClassifier:
    """Get ICICI classifier instance."""
    return ICICIClassifier()


def classify(row) -> Tuple[str, int]:
    """Classify a transaction row using the shared base classifier path."""
    return _base_classify(row, classifier_factory=get_classifier, bank_name='ICICI')
...
def generate_report(
    ...
    """Generate the shared HDFC-style report for ICICI statements."""
